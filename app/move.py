...
def check_move(x, y, best_move, board_matrix, height) :
    ...
    # Check up  added and y<0 and others
    if y - 1 > 0 and board_matrix[y-1][x] == OCCUPIED and y < 0 :
        sum_move = sum_move + 1


    # Check down
    if y + 1 < (height - 1) and board_matrix[y+1][x] == OCCUPIED and y > (height-1) : #added the minus 1
        sum_move = sum_move + 1
    ...
    # check right
    if x + 1 < (height - 1) and  board_matrix[y][x+1]== OCCUPIED and x > (height-1) : #added the minus 1
        sum_move = sum_move + 1
